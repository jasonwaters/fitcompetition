...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            accounts = db.execute("SELECT * FROM fitcompetition_account;")

        # Deleting field 'Account.user'
        db.delete_column(u'fitcompetition_account', 'user_id')

        # Deleting field 'Account.challenge'
        db.delete_column(u'fitcompetition_account', 'challenge_id')

        # Adding field 'FitUser.account'
        db.add_column(u'fitcompetition_fituser', 'account',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['fitcompetition.Account']),
                      keep_default=False)

        # Adding field 'Challenge.account'
        db.add_column(u'fitcompetition_challenge', 'account',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['fitcompetition.Account']),
                      keep_default=False)
        ...
        if not db.dry_run:
            for accountDetails in accounts:
                ...
                accountDescription = accountDetails[1]
                userID = accountDetails[2]
                challengeID = accountDetails[3]

                account = orm.Account.objects.get(id=accountID)

                if userID is not None:
                    user = orm.FitUser.objects.get(id=userID)
                    user.account = account
                    user.save()

                if challengeID is not None:
                    challenge = orm.Challenge.objects.get(id=challengeID)
                    challenge.account = account
                    challenge.save()

    def backwards(self, orm):
        # Deleting field 'FitUser.account'
        db.delete_column(u'fitcompetition_fituser', 'account_id')

        # Adding field 'Account.user'
        db.add_column(u'fitcompetition_account', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['fitcompetition.FitUser'], null=True, blank=True),
                      keep_default=False)

        # Adding field 'Account.challenge'
        db.add_column(u'fitcompetition_account', 'challenge',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['fitcompetition.Challenge'], null=True, blank=True),
                      keep_default=False)

        # Deleting field 'Challenge.account'
        db.delete_column(u'fitcompetition_challenge', 'account_id')
    ...
    models = {
        ...
        u'fitcompetition.fitnessactivity': {
            ...
            'hasEvidence': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
