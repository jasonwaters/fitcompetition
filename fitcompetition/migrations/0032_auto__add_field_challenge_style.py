...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Adding field 'Challenge.style'
        db.add_column(u'fitcompetition_challenge', 'style',
                      self.gf('django.db.models.fields.CharField')(default='ALL', max_length=6),
                      keep_default=False)

        if not db.dry_run:
            challenges = orm.Challenge.objects.all()
            for challenge in challenges:
                if challenge.type == 'SIMP':
                    challenge.type = 'INDV'
                    challenge.style = 'ALL'
                elif challenge.type == 'TEAM':
                    challenge.style = 'ONE'

                challenge.save()

    def backwards(self, orm):
        # Deleting field 'Challenge.style'
        db.delete_column(u'fitcompetition_challenge', 'style')
