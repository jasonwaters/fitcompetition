...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            fitusers = orm.FitUser.objects.all()
            for fituser in fitusers:
                orm.Account.objects.create(description="User Account: %s" % fituser.fullname,
                                           user=fituser)


            challenges = orm.Challenge.objects.all()
            for challenge in challenges:
                orm.Account.objects.create(description="Challenge Account: %s" % challenge.name,
                                           challenge=challenge)

            orm.Account.objects.create(description="Master Account: Fit Crown")
