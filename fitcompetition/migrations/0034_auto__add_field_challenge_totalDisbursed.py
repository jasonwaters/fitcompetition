...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Challenge.totalDisbursed'
        db.add_column(u'fitcompetition_challenge', 'totalDisbursed',
                      self.gf('fitcompetition.models.CurrencyField')(default=0, null=True, max_digits=16, decimal_places=2, blank=True),
                      keep_default=False)

        if not db.dry_run:
            challenges = orm.Challenge.objects.filter(reconciled=True)
            for challenge in challenges:
                challenge.totalDisbursed = challenge.disbursementAmount * challenge.numWinners
                challenge.save()

    def backwards(self, orm):
        # Deleting field 'Challenge.totalDisbursed'
        db.delete_column(u'fitcompetition_challenge', 'totalDisbursed')
