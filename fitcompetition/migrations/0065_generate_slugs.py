...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            challenges = orm.Challenge.objects.all()
            for challenge in challenges:
                slug = slugify(challenge.name)
                try:
                    existing = orm.Challenge.objects.get(slug=slug)
                    slug = slug + u'-' + unicode(challenge.enddate.strftime('%Y-%m-%d'))
                except orm.Challenge.DoesNotExist:
                    pass
